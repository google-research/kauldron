...
class _Sharding:
  ...
  def device_put_v2(
      self, arrays: _T, sharding: jax.sharding.NamedSharding  # pylint: disable=redefined-outer-name
  ) -> _T:
    ...
    """
    return _get_jit_for_sharding(sharding)(arrays)
...
@functools.cache
def _get_jit_for_sharding(sharding):  # pylint: disable=redefined-outer-name
