...
class PatchConfig:
  ...
  def __call__(self, cfg: konfig.ConfigDict) -> dict[str, Any]:
    """Modifies the config in-place and returns a dict of applied updates."""
    updates = {}
    if self.stop_after_steps is not None:
      cfg.stop_after_steps = self.stop_after_steps
      updates["stop_after_steps"] = self.stop_after_steps

    self._patch_batch_size(cfg)  # TODO(klausg): add to updates

    if self.skip_eval:
      cfg.evals = {}
      updates["evals"] = {}
    else:
      self._patch_num_batches(cfg)  # TODO(klausg): add to updates
      self._patch_eval_run_once(cfg)  # TODO(klausg): add to updates

    if self.skip_checkpointer:
      cfg.checkpointer = None
      updates["checkpointer"] = None

    if self.skip_metrics:
      cfg.train_metrics = {}
      updates["train_metrics"] = {}

    if self.skip_summaries:
      cfg.train_summaries = {}
      updates["train_summaries"] = {}

    if hasattr(cfg, "setup"):
      cfg.setup.add_flatboard = not self.skip_flatboard
      updates["setup.add_flatboard"] = not self.skip_flatboard

    self._patch_profile(cfg)  # TODO(klausg): add to updates
    self._patch_checkify(cfg)  # TODO(klausg): add to updates
  ...
  def _patch_eval_run_once(self, cfg: konfig.ConfigDict) -> None:
    ...
    for _, eval_cfg in cfg.evals.items():
      eval_cfg.run = kd_evals.Once(0)  # NOTE: kd_evals is a konfig.import
