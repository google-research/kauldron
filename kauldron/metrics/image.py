...
@dataclasses.dataclass(kw_only=True, frozen=True, eq=True)
class DepthRGBMetrics(base.Metric):
  ...
  def get_state(
    ...
    pred = _invert_preprocess_depth(pred, colormap)
    target = _invert_preprocess_depth(target, colormap)

    pred = jnp.clip(pred, self.min_val, self.max_val)
    target = jnp.clip(target, self.min_val, self.max_val)

    abs_rel = jnp.abs(pred - target) / target
