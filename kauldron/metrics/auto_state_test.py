...
class _MergeFieldInnerState(auto_state.AutoState):
...
@flax.struct.dataclass(kw_only=True)
class _MergeFieldOuterState(auto_state.AutoState):
  inner: _MergeFieldInnerState = auto_state.merge_field()
...
def test_merge_field():
  s1 = _MergeFieldOuterState(
      inner=_MergeFieldInnerState(x=jnp.array(1.0)), y=jnp.array(2.0)
  )
  s2 = _MergeFieldOuterState(
      inner=_MergeFieldInnerState(x=jnp.array(10.0)), y=jnp.array(20.0)
  )

  s_merged = s1.merge(s2)
  assert isinstance(s_merged.inner, _MergeFieldInnerState)
