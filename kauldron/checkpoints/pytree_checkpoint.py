...
# TODO(epot): Replace by `StandardCheckpointHandler`
...
class PyTreeCheckpointHandler(ocp.PyTreeCheckpointHandler):
  ...
  def restore(  # pytype: disable=signature-mismatch
    ...
    Returns:
      The restored item.
    """
    if self.metadata(directory) is None:  # Legacy checkpoint
      print(
          'Loading DEPRECATED checkpoint (values flattened). Kauldron will '
          'drop support for those at some point.'
      )
      # Untested, not sure this works as the `TrainState.tree_flatten` has
      # changed

      target, structure = jax.tree_util.tree_flatten(item)
      ...
      return jax.tree_util.tree_unflatten(structure, state)
