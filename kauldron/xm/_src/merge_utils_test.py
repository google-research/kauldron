...
def test_merge_job_subclass_missing_dataclass():
  ...
  # Pass-through subclasses adding only methods do not require @dataclass.
  class PassThroughJob(kxm.Job):

    def helper(self) -> str:
      return 'ok'

  merged_pass_through = merge_utils.merge(
      PassThroughJob(target='//path/to:target'),
      job_params.JobParams(platform='v4'),
  )
  assert isinstance(merged_pass_through, PassThroughJob)
  assert merged_pass_through.platform == 'v4'
  assert merged_pass_through.helper() == 'ok'
