...
def resolve(cfg, *, freeze=False):
  ...
  Args:
    ...
    freeze: (Deprecated) If `True`, `list` are converted to `tuple`, `dict`/`ConfigDict` are
      converted to `immutabledict`. Default to `False`.
  ...
  if '_konfig_experimental_nofreeze' in cfg:
    warnings.warn(
        '`_konfig_experimental_nofreeze` is deprecated. Default behavior is now no freeze.',
        DeprecationWarning,
        stacklevel=2,
    )
    cfg = copy.copy(cfg)
