...
class ModuleConfigDict(AutoNestedConfigDict):
  ...
  def module_config(self) -> konfig.ConfigDict:
    ...
    # save config args in config for access when reloading
    if not isinstance(self.config_args, str):
      if not hasattr(config, "aux"):
        config.aux = konfig.ConfigDict()
      config.aux.__args__ = dataclasses.asdict(self.config_args)
