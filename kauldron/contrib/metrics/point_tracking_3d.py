...
class Tap3DPositionAccuracy(metrics.Metric):
  ...
  def get_state(
    ...
    if evaluation_mask is not None:
      evaluation_mask_bool = evaluation_mask.astype(bool)
      batch_ndim = values.ndim
      reduce_axis = tuple(range(batch_ndim, evaluation_mask_bool.ndim))
      batch_mask = jnp.any(evaluation_mask_bool, axis=reduce_axis)
